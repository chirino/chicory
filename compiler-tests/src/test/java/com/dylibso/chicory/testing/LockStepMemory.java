...
public final class LockStepMemory implements Memory {
    ...
    public Object lock(int address) {
        Object lock = memory.lock(address);
        exchange("lock", address);
        return lock;
    }

    @Override
    public int waitOn(int address, int expected, long timeout) {
        int result = memory.waitOn(address, expected, timeout);
        exchange("waitOn", result, address, expected, timeout);
        return result;
    }
    ...
    public int notify(int address, int maxThreads) {
        int result = memory.notify(address, maxThreads);
        exchange("notify", result, address, maxThreads);
        return result;
    }
